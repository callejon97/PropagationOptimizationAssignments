...
int main()
{
    ...
    std::string outputPath = tudat_applications::getOutputPath( "ShapeOptimizationQ3" );
            ...
            for( unsigned int j = 0; j < numberOfIntegrators; j++ )
            {
                ...
                for( unsigned int k = 0; k < numberOfIntegratorStepSizeSettings; k++ )
                {
                    ...
                    outputPath = tudat_applications::getOutputPath(
                                "ShapeOptimizationQ3/prop_" + std::to_string( i ) + "/int_" + std::to_string( j ) + "/setting_" + std::to_string( k ) + "/" );
